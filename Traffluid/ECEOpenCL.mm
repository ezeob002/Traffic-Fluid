...
/**
 * It initializes and sets up opengCL initial configuration.
 *
 * @param
 * @return
 */
- (int) initOpenCL
{
    ...
    // Enumarating the List of Platforms
    displayInfo();
...
/*
 * Enumarates the List of Platforms
 *
 *
 */
///
...
///
//  Querying and Displaying Platform-Specific Information
//
...
cl_context CreateContext()
{
    ...
    if (errNum != CL_SUCCESS || numPlatforms <= 0)
    {
        NSLog(@"Failed to find any OpenCL platforms.");
        return NULL;
    }
    ...
    if (errNum != CL_SUCCESS)
    {
        NSLog(@"Could not create GPU context, trying CPU...");
        context = clCreateContextFromType(contextProperties, CL_DEVICE_TYPE_CPU,
                                          NULL, NULL, &errNum);
        if (errNum != CL_SUCCESS)
        {
            NSLog(@"Failed to create an OpenCL GPU or CPU context.");
            return NULL;
        }
    }
...
cl_command_queue CreateCommandQueue(cl_context context, cl_device_id *device)
{
    ...
    if (errNum != CL_SUCCESS)
    {
        NSLog(@"Failed call to clGetContextInfo(...,GL_CONTEXT_DEVICES,...)");
        return NULL;
    }
    ...
    devices = (cl_device_id*)malloc(sizeof(cl_device_id) * deviceBufferSize / sizeof(cl_device_id));
    
    errNum = clGetContextInfo(context, CL_CONTEXT_DEVICES, deviceBufferSize, devices, NULL);
    if (errNum != CL_SUCCESS)
    {
        free (devices);
        NSLog(@"Failed to get device IDs");
        return NULL;
    }
    ...
    {
        free(devices);
        NSLog(@"Failed to create commandQueue for device 0");
        return NULL;
    }
